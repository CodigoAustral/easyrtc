...
pub.app = function(appName, callback) {
    ...
    appObj.connection = function(easyrtcid, callback) {
         ...
         * @param       {function(?Error, Object=)} callback Callback with error and roomList object.
         */
        connectionObj.returnUsersList = function(customer, callback) {
            pub.util.logDebug("[" + appName + "][" + easyrtcid + "] Running func 'connectionObj.returnUsersList'");
            
            var memcached = new Memcached('127.0.0.1:11211');
            memcached.get( "users-" + customer, function( err, result ){
            ...
            memcached.get( "room-" + roomName, function( err, result ){
                ...
                for (targetId in usersInRoom) {
                    for (index in e.app[appName].room[currentUserCustomer].clientList) {
                        ...
                        if (e.app[appName].room[currentUserCustomer].clientList[index].userId == usersInRoom[targetId] && typeof e.app[appName].room[roomName].clientList[index] == "undefined") {
                            callback(null, index);
                            return false;
                        } 
                    }
