...
pub.events.defaultListeners = {
    ...
    "roomUpdateConnections": eventListener.onRoomUpdateConnections,
    ...
    "usersList": eventListener.onUsersList
};
...
pub.app = function(appName, callback) {
    ...
    appObj.connection = function(easyrtcid, callback) {
            ...
            for (var currentRoomName in e.app[appName].connection[easyrtcid].room) {
                ...
                for (var currentEasyrtcid in connectionRoom.toRoom.clientList) {
                    ...
                    if (currentToConnection.username) {
                        ...
                        var userData = JSON.parse(currentToConnection.username);
                        roomData[currentRoomName].clientList[currentEasyrtcid].userId = userData.u;
                    }
                }
         ...
         * @param       {function(?Error, Object=)} callback Callback with error and roomList object.
         */
        connectionObj.returnUsersList = function(roomName, callback) {
            pub.util.logDebug("[" + appName + "][" + easyrtcid + "] Running func 'connectionObj.returnUsersList'");
            
            var memcached = new Memcached('127.0.0.1:11211');
            memcached.get( "room-" + roomName, function( err, result ){
        ...
        /**
         * Makes the recipients join a room
         *
         * @memberof    pub.appObj.connectionObj
         * @param       {function(?Error, Object=)} callback Callback with error and roomList object.
         */
        connectionObj.updateRoomConnections = function(roomName, callback) {
            pub.util.logDebug("[" + appName + "][" + easyrtcid + "] Running func 'connectionObj.updateRoomConnections'");
            var memcached = new Memcached('127.0.0.1:11211');
            var currentUserId = JSON.parse(e.app[appName].connection[easyrtcid].username).u;
            var currentUserCustomer = JSON.parse(e.app[appName].connection[easyrtcid].username).c;
            var targetId, index, removeUser, msg;
            memcached.get( "room-" + roomName, function( err, result ){
                if( err ) console.error( err );
                memcached.end(); // as we are 100% certain we are not going to use the connection again, we are going to end it

                var usersInRoom = JSON.parse(result);
                // Check if the server has to remove a user from the room
                for (index in e.app[appName].room[roomName].clientList) {
                    if (index == easyrtcid) continue;
                    removeUser = true;
                    for (targetId in usersInRoom) {
                        if (e.app[appName].room[roomName].clientList[index].userId == usersInRoom[targetId].uid) {
                            removeUser = false;
                        } 
                    }
                    if (removeUser && typeof usersInRoom[targetId] != "undefined") {
                        msg = {
                            action: 'remove',
                            targetId: index
                        }
                ...
                // Check if the server has to add a user to the room
                for (targetId in usersInRoom) {
                    for (index in e.app[appName].room[currentUserCustomer].clientList) {
                        if (index == easyrtcid) continue;
                        if (e.app[appName].room[currentUserCustomer].clientList[index].userId == usersInRoom[targetId].uid && typeof e.app[appName].room[roomName].clientList[index] == "undefined") {
                                msg = {
                                action: 'add',
                                targetId: index
                            }
        ...
        connectionObj.joinRoom = function(roomName, callback) {
            ...
            var createConnectionRoom = function(roomName, appRoomObj, callback) {
                ...
                var userData = JSON.parse(e.app[appName].connection[easyrtcid].username);
