...
eventListener.onConnection = function(socket, easyrtcid, next){
    ...
    socket.on("easyrtcAuth", function(msg, socketCallback) {
        ...
        if (typeof disconnects.userId !== "undefined") {
            //var connectionObj = disconnects.userId.connection;
            //socket.id = connectionObj.id;
            pub.util.logDebug("["+easyrtcid+"] Removing 'disconnect' timeout and canceling the new connection");
            clearTimeout(disconnects.userId.timer);
        ...
        function(asyncCallback) {
            ...
            switch(msg.msgType) {
                ...
                case "roomUpdateConnections":
                    pub.events.emit("msgTypeRoomUpdateConnections", connectionObj, msg.msgData.roomName, socketCallback, next);
                    break;
                case "roomLeave":
                ...
                case "getUsersList":
                    pub.events.emit("usersList", connectionObj, msg.msgData.customer, socketCallback, next);
                    break;
...
/**
 * Default listener for event "msgTypeRoomUpdateConnections". 
 * 
 * @param       {Object} connectionObj  EasyRTC connection object. Contains methods used for identifying and managing a connection.
 ...
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onMsgTypeRoomUpdateConnections = function(connectionObj, roomName, socketCallback, next){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getEasyrtcid()+"] Running func 'onMsgTypeRoomJoin'");
    if(!_.isFunction(socketCallback)) {
        pub.util.logWarning("["+connectionObj.getAppName()+"]["+connectionObj.getEasyrtcid()+"] EasyRTC info: unhandled socket message callback.");
        return;
 ...
 * @param       {Function} callback     Callback of form (err, connectionRoomObj)
 */
eventListener.onRoomUpdateConnections = function(connectionObj, roomName, roomParameter, callback){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getEasyrtcid()+"] Running func 'onRoomJoin'");
 ...
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onUsersList = function(connectionObj, customer, socketCallback, next){
    ...
    connectionObj.returnUsersList(customer, 
