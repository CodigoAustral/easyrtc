...
eventListener.onDisconnect = function(connectionObj, next){
    var userId = JSON.parse(connectionObj.getUsername()).u;
    disconnects.userId = {
        'timer': setTimeout(
            function(){
...
eventListener.onEasyrtcAuth = function(socket, easyrtcid, msg, socketCallback, callback){
    ...
    var userId = JSON.parse(msg.msgData.username).u;
        ...
        function(asyncCallback) {
            ...
            switch(msg.msgType) {
                ...
                case "roomJoin":
                    ...
                    break;
                case "roomUpdateConnections":
                    pub.events.emit("roomUpdateConnections", connectionObj, msg.msgData.roomName, socketCallback, next);
                    break;
                case "roomLeave":
                ...
                case "getUsersList":
                    pub.events.emit("usersList", connectionObj, msg.msgData.roomName, socketCallback, next);
                    break;
        ...
        function(asyncCallback) {
            ...
            if (msg.targetEasyrtcid !== undefined) {
                ...
                var outgoingMsg = {
                    ...
                    username: JSON.parse(connectionObj.getUsername()).n,
                    msgType: msg.msgType,
                    msgData: msg.msgData
                };
...
/**
 * Default listener for event "usersList". Run upon a room update
 * 
 * @param       {string} connectionObj  EasyRTC connection object. Contains methods used for identifying and managing a connection.
 * @param       {Object} msg            Message object which contains the full message from a client; this can include the standard msgType and msgData fields.
 * @param       {Function} socketCallback Socket.io callback function which delivers a response to a socket. Expects a single parameter (msg).
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onUsersList = function(connectionObj, roomName, socketCallback, next){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getEasyrtcid()+"] Running func 'onUsersList'");
    ...
    connectionObj.returnUsersList(roomName, 
        function(err, usersList) {
            if(err) {
                socketCallback(pub.util.getErrorMsg("MSG_REJECT_NO_USERS_LIST"));
            }
            else {
                socketCallback({"msgType":"usersList", "msgData":{"usersList":usersList}});
            }
            next(null);
        }
    );
...
/**
 * Default listener for event "roomUpdateConnections". Adds the users in a room as connections. This is called when a new room is created or when a user is added/removed from a room.
 * 
 * @param       {Object} connectionObj  EasyRTC connection object. Contains methods used for identifying and managing a connection.
 * @param       {string} roomName       Room name which uniquely identifies a room within an EasyRTC application.
 * @param       {?Object} roomParameter A map(dictionary) object with key/value pairs. The values can be any JSONable object. This field is not currently looked at by EasyRTC, however it is available for custom server applications. May be used for room options or authentication needs.
 * @param       {Function} callback     Callback of form (err, connectionRoomObj)
 */
eventListener.onRoomUpdateConnections = function(connectionObj, roomName, socketCallback, next){
    pub.util.logDebug("["+connectionObj.getAppName()+"]["+connectionObj.getEasyrtcid()+"] Running func 'onRoomUpdateConnections'");
    ...
    connectionObj.updateRoomConnections(roomName, 
        function(err, msg) {
            // en lugar de targetId podría usar un objeto msg con value=targetId y action=remove/add
            if(err) {
                socketCallback(pub.util.getErrorMsg("MSG_REJECT_NO_USERS_LIST"));
            }
            else {
                pub.getConnectionWithEasyrtcid(msg.targetId, function(err, targetConnection) {
                    if (err) {
                        callback(err);
                        return;
                    }
                    if (msg.action == "add") {
                        targetConnection.joinRoom(roomName, function(err, connectionRoomObj){
                            if (err) {
                                callback(err);
                                return;
                            }
                        ...
                        targetConnection.room(roomName, function(err, connectionRoomObj){
                            if (err) {
                                socketCallback(pub.util.getErrorMsg("MSG_REJECT_BAD_ROOM"));
                                next(null);
                                return;
                            ...
                            connectionRoomObj.leaveRoom(socketCallback({"success":"true"}));
                        });
                    }
                });
            }
